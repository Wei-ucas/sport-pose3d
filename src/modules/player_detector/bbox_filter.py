...
class BboxFilter:
    ...
    def set_camera_params(self, camera_params):
        ...
        self.camera_params["inv_P"] = (
            np.linalg.inv(self.camera_params["RT"][:, [0, 1, 3]])
            @ self.camera_params["invK"]
        )
        self.view_id = 0
    ...
    def filter(
        self,
        bboxes: np.ndarray,
        camera_params: Dict[str, np.ndarray] = None,
        court_crop_range: List[float] = None,
    ) -> List[bool]:
        """
        Filter the bbox with the ground plane, bbox not in the court will be filtered out.
        Args:
        ...
        if camera_params is not None:
            self.set_camera_params(camera_params)
            # print("update camera")
        bboxes_bottom_center = np.concatenate(
            [(bboxes[:, :1] + bboxes[:, 2:3]) / 2, bboxes[:, 3:4]], axis=1
        )
        ground_points = image2ground(bboxes_bottom_center, self.camera_params)
        valid_idx = np.logical_and(
            np.logical_and(
                np.logical_and(
                    ground_points[:, 0] > self.court_range_x[0],
                    ground_points[:, 0] < self.court_range_x[1],
                ),
                np.logical_and(
                    ground_points[:, 1] > self.court_range_y[0],
                    ground_points[:, 1] < self.court_range_y[1],
                ),
            ),
    ...
    def remove_duplicate_k2d(self, k2d_dets: np.ndarray):
        ...
        k3d_dist_mat = np.linalg.norm(
            k2d_dets[:, None, :, :2] - k2d_dets[None, :, :, :2], axis=-1
        )
