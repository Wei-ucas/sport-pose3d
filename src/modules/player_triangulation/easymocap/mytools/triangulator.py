...
def project_and_distance(kpts3d, RT, kpts2d):
    ...
    return dist, conf, kpts_proj

def iterative_triangulate(kpts2d, RT, previous=None,
    min_conf=0.1, min_view=3, min_joints=1, dist_max=0.05, dist_vel=0.05,
    thres_outlier_view=0.4, thres_outlier_joint=0.4, debug=False):
    # TODO(Xiao): check ball associate parameters
    kpts2d = kpts2d.copy()
    conf = kpts2d[..., -1]
    ...
    if previous is not None:
        dist, conf, kpts_proj = project_and_distance(previous, RT, kpts2d)
    ...
    while True:
        ...
        dist, conf, kpts_proj = project_and_distance(kpts3d, RT, kpts2d)
        # 2. find the outlier
    ...
    if (kpts3d[..., -1]>0).sum() < min_joints:
        kpts3d[..., -1] = 0.
        kpts2d[..., -1] = 0.
        return kpts3d, kpts2d, dist, kpts_proj
    return kpts3d, kpts2d, dist, kpts_proj
