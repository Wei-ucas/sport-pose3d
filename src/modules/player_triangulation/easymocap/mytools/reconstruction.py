...
def projectN3(kpts3d, Pall):
    # kpts3d: (N, 3)
    assert  kpts3d.shape[1] == 4, "Need 3D keypoints with confidence!"
    nViews = len(Pall)
    kp3d = np.hstack((kpts3d[:, :3], np.ones((kpts3d.shape[0], 1))))
    # kp3d = np.hstack((kpts3d[:, :3]))
    kp2ds = []
...
def batch_triangulate(keypoints_, Pall, keypoints_pre=None, lamb=1e3, kpt_thr=0.3):
    # keypoints: (nViews, nJoints, 3)
    # Pall: (nViews, 3, 4)
    # A: (nJoints, nViewsx2, 4), x: (nJoints, 4, 1); b: (nJoints, nViewsx2, 1)
    v = (keypoints_[:, :, -1]>kpt_thr).sum(axis=0)
