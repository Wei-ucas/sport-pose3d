...
def simple_associate(annots, affinity, dimGroups, Pall, group, cfg, images=None, kpt_thr=0.3):
    nViews = len(annots)
    ...
    for idx in sortidx:
        ...
        while len(proposals) > 0:
            ...
            keypoints3d = batch_triangulate(keypoints2d, Pused, kpt_thr=kpt_thr)
            kptsRepro = projectN3(keypoints3d, Pused)
            err = ((kptsRepro[:, :, 2]>kpt_thr)*(keypoints2d[:, :, 2]) > kpt_thr) * np.linalg.norm(kptsRepro[:, :, :2] - keypoints2d[:, :, :2], axis=2)
        ...
        if len(results) > 1:
            ...
            results.sort(key=lambda x:len(x['Vused']), reverse=True)
        result = results[0]
