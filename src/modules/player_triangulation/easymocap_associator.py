...
from .utils import player_to_easymocap_format, camera_to_easymocap_format, easymocap_result_to_player
...
class Affinity(ComposedAffinity):
    affinity_cfg = edict({
        'aff_min': 0.2, 'svt_py': False,
        'aff_funcs': edict({'easymocap.affinity.ray.Affinity': edict({'MAX_DIST': 0.1})}),
        'svt_args': edict({'debug': 0, 'log': 0, 'maxIter': 20, 'w_sparse': 0.1, 'w_rank': 50, 'tol': 0.0001}),
        'vis_aff': False, 'vis_res': False, 'vis_pair': False
    })
...
class PlayerTriangulator(BaseModule):
    associate_cfg = edict({
        'debug': False, 'log': False, 'body': 'body25', 'max_repro_error': 0.1, 'min_views': 2,
        'criterions': edict({
            'easymocap.assignment.criterion.BaseCrit': edict({'min_conf': 0.1, 'min_joints': 3}),
            'easymocap.assignment.criterion.CritLenTorso': edict(
                {'src': 2, 'dst': 5, 'min_torso_length': 0.1, 'max_torso_length': 1, 'min_conf': 0.1}),
    ...
    def __init__(self, cameras_params, camera_ids, pose_conversion='coco17', dist_max=25,
                 num_players=10):
        ...
        assert pose_conversion == 'coco17', "Only coco17 pose conversion is supported in EasyMocapAssociate"
    ...
    def process(self, mv_frame: MvFrame) -> MvFrame:
        ...
        if self.cameras is None:
            assert camera_params is not None, "Camera parameters must be provided if cameras are not initialized."
        ...
        mv_mp_player_list: Dict[str, List[Player]] = {cam: mv_frame.frame_dict[cam].players for cam in self.camera_ids}
        ...
        for result in results:
            ...
            reid_distances.append(distances.min(axis=0)) # for each player, use the minimum distance across all cameras
        ...
        for camera_id, annots in camera_annots.items():
            for p in annots:
                if np.sum(p['keypoints'][:, -1]) > 0:
                    p['keypoints'] = undistort_points(p['keypoints'], self.cameras[camera_id]["K"],
    ...
    def associate_triangulate(self, camera_annots, images):
        ...
        associate_results = simple_associate(annots, affinity, dimGroups, self.Pall,
                                             PeopleGroup(Pall=self.Pall, cfg=edict({})), cfg=self.associate_cfg,
        ...
        for ik, (pid, people) in enumerate(associate_results.items()):
            ...
            result = {'id': pid, 'keypoints3d': people.keypoints3d, "bbox": people.info['bbox'],
                      "kptsRepro": people.info['kptsRepro'], "indices": people.indices}
