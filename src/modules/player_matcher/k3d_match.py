    ...
    for i, (kps, confidence) in enumerate(sorted_candidates):
        if i in rest_index:
            ...
            for j in rest_index:
                ...
                overlap = current_overlap + (sorted_candidates[j][0][:, 2] != 0).astype(
                    "int"
                )
...
class Player3DMatcher(BaseModule):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Player3DMatcher initialized with track_distance={track_distance}, "
        )
    ...
    def matching_frame_clip(self, mv_frames: List[MvFrame]):
        ...
        for i, frame in enumerate(mv_frames):
            for player_id, player_loc in frame.tracked_player_location.items():
                ...
                tracking_players_reid[player_id][i] = frame.tracked_player_reid_info[
                    player_id
                ]
        tracking_player_appear = {}
        for player_id in tracking_players.keys():
            tracking_player_appear[player_id] = (
                tracking_players[player_id][:, 2] != 0
            ).sum(0)
        sorted_tracking_player_appear = sorted(
            list(tracking_player_appear.keys()),
            key=lambda x: tracking_player_appear[x],
            reverse=True,
        )
        ...
        for player_id in sorted_tracking_player_appear:
            ...
            if len(valid_ids) > 0 and n_frame_player_appear > 25:
                ...
                for i in range(len(id_count)):
                    id_confidence[i] = (
                        self.reid_dist_threshold / np.mean(cost[pid == i])
                        if id_count[i] > 0
                        else 0
                    )
                ...
                match_confidence = (
                    max_count
                    / sum(id_count)
                    * (self.reid_dist_threshold / mean_cost) ** 2
                    * np.sqrt(
                        len(valid_ids) / self.appear_frame_thr
                        if len(valid_ids) < self.appear_frame_thr * 2
                        else 2
                    )
                    * np.sqrt(len(valid_ids) / n_frame_player_appear)
                )
        ...
        for reid_id in reid_player_candidates.keys():
            ...
            keep_candidates, remove_candidates = trajectory_nms(
                reid_player_candidates[reid_id]
            )
            ...
            for i in range(1, len(keep_candidates)):
                if (
                    (reid_players[reid_id][:, 2] != 0).astype("int")
                    + (keep_candidates[i][:, 2] != 0).astype("int")
                ).max() > 1:
                    self.logger.warn(
                        f"Player {reid_id} has overlapping frames, merging them."
                    )
        ...
        for i, frame in enumerate(mv_frames):
            for player_id, player_loc in reid_players.items():
                frame.matched_player_k3d[player_id] = player_loc[i]
            for k3d in no_name_players:
                frame.unmatched_player_k3d.append(k3d[i])
            frame.tracked_player_location = {}
