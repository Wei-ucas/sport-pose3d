...
def multiview_frame_reader(data_path_cfg: GamePath, view_time_offset: Dict[int, int] = None):
    """
    ...
    for view_id in data_path_cfg.view_list:
        frame_reader = FrameInput(
            view_id=view_id,
            pkl_data_path=data_path_cfg.get_reid_path(view_id),
...
def get_video_sync(data_path_cfg: GamePath):
    ...
    if not os.path.exists(video_sync_result_path):
        raise FileNotFoundError(f"Video sync result file not found: {video_sync_result_path}")
...
def triangulation_processor_raw(
    ...
    if not os.path.exists(raw_3d_save_path):
        ...
        mv_frame_reader = multiview_frame_reader(data_path_cfg, view_time_offset=view_frame_offset)
    else:
    ...
    logger.info(f"Triangulation processing completed. Raw 3D data saved to {raw_3d_save_path}")
...
def triangulation_processor_match(
    ...
    if not os.path.exists(data_path_cfg.get_prediction_save_path(prediction_type="3d-raw")):
        logger.error("Raw 3D data not found. Please run triangulation_processor_raw first.")
        return
    ...
    if not os.path.exists(matched_3d_save_path):
        ...
        mv_frame_reader = MvFrameInput(data_path_cfg.get_prediction_save_path(prediction_type="3d-raw"))
    else:
    ...
    for i, mv_frame in tqdm.tqdm(enumerate(mv_frame_reader), desc="Processing multiview frames with matching"):
        if mv_frame is None:
            break
        # Match players across frames
    ...
    logger.info(f"Triangulation processing with matching completed. Matched 3D data saved to {matched_3d_save_path}")


def triangulation_processor(
        data_path_cfg: GamePath,
):
    """
    Main function to process triangulation and matching.
    """
    triangulation_processor_raw(data_path_cfg)
    triangulation_processor_match(data_path_cfg)
    logging.getLogger("TriangulationProcessor").info("Triangulation processing completed successfully.")