"""
将 video_sync.json 的同步结果写回 mocap-project JSON 文件中对应的相机记录。
...
def main():
    ...
    if args.sync_json:
        ...
        jobs = [(args.sync_json, sanitize_name(group_name), segment_key.lower())]
    ...
    if total_updated > 0 and not args.dry_run:
        ...
        print(f"\n[模拟] 共涉及 {total_updated} 条记录，未写入任何文件")
    else:
        print("\n[信息] 没有更新任何记录")

    if args.export_root:
        if args.workdirs_root:
            export_source_root = args.workdirs_root
        elif args.workdir:
            export_source_root = os.path.dirname(os.path.abspath(args.workdir))
        else:
            workdir_name = jobs[0][1] if jobs else None
            sync_abs = os.path.abspath(args.sync_json)
            export_source_root = os.path.dirname(
        ...
        exported = export_segment_outputs(
            project,
            os.path.abspath(export_source_root),
